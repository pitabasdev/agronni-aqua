...
const thirdnode = require("./model/thirdnode");
const secondnode = require("./model/secondnode");
//
const fs = require("fs");
...
app.get("/getrestsensordata/:id", (req, res) => {
  secondnode
    .find({ aquaid: req.params.id }, null, { sort: "-date" }, (err, result) => {
      if (!err) {
        if (result) {
          res.json(result);
        }
  ...
  async (req, res) => {
    ...
    try {
      const response = await sensorinfoModel.create({
        aquid: req.params.id,
        temp: req.params.v1,
        waterlevel: req.params.v2,
        turbidity: req.params.v3,
        flow: req.params.v4,
      });
...
app.get("/addsensorsecond/:id/val1/:vl", async (req, res) => {
  try {
    const response = await secondnode.create({
      aquaid: req.params.id,
      ph: req.params.vl,
    });
    console.log(response);
    res.json({ status: "ok", message: "Successfully Inserted PH" });
  } catch (error) {
    console.error("Error:", error);
    res.json({ status: "fail", message: "Something Went Wrong!" });
  }
});
app.get("/addSecondnode/:id/val1/:vl", async (req, res) => {
  try {
    const response = await secondnode.create({
      aquaid: req.params.id,
      ph: req.params.vl,
...
app.get("/addsthird/:id/val1/:vl", async (req, res) => {
  try {
    const response = await secondnode.create({
      aquaid: req.params.id,
      do: req.params.vl,
    ...
    console.log(response);
    res.json({ status: "ok", message: "Successfully Inserted DO" });
  } catch (error) {
    console.error("Error:", error);
    res.json({ status: "fail", message: "Something Went Wrong!" });
  }
